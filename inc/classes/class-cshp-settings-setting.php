...
class CSHP_Settings_Setting {
	...
	public function __construct( $args, $section_id, $menu_id ) {
		...
		add_action( 'admin_enqueue_scripts', array( &$this, 'square_field_admin_enqueue_scripts' ) );
	}

	/**
	 * Enqueue admin scripts for our settings.
	 */
	public function square_field_admin_enqueue_scripts() {
		if ( strpos( $this->classes, 'enhanced' ) !== false ) :
			wp_enqueue_script( 'swpe-select2' );
			wp_enqueue_style( 'swpe-select2' );
			wp_enqueue_script( 'swpe-enhanced-fields' );
		endif;
	}
